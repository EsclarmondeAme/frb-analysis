#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
frb remnant-time phase-memory — quadrant-by-quadrant real vs isotropic (test 85M)
---------------------------------------------------------------------------------

goal:
    test whether the remnant-time phase-memory signal persists *locally*
    when the sky is carved into four galactic quadrants. this is stronger
    than 85J/85K/85L: the sky is no longer global, no axial symmetry, and
    no large-scale coherence. the question:

        does each quadrant individually show excess phase-memory
        compared to isotropic skies restricted to the same chunk?

procedure:
    - convert FRBs to galactic (l,b)
    - define galactic quadrants:
         Q1: 0≤l<180, b≥0
         Q2: 0≤l<180, b<0
         Q3: 180≤l<360, b≥0
         Q4: 180≤l<360, b<0
    - for each quadrant:
         * select FRBs
         * compute Z_real
         * generate isotropic skies restricted to that quadrant
         * compute Z_iso for each
         * p_geom = #Z_iso >= Z_real / N_iso

interpretation:
    if Z_real > isotropic mean in each quadrant and p_geom is small,
    the signal is *spatially distributed* and not localized.
"""

import numpy as np
import csv
import sys
import math
from tqdm import tqdm

# ------------------------------------------------------------
# catalog utilities
# ------------------------------------------------------------

def detect_columns(fieldnames):
    low = [c.lower() for c in fieldnames]

    def find(*names):
        for n in names:
            if n.lower() in low:
                return fieldnames[low.index(n.lower())]
        return None

    ra = find("ra_deg","ra","raj2000","ra (deg)")
    dec = find("dec_deg","dec","dej2000","dec (deg)")
    if ra is None or dec is None:
        raise KeyError("RA/Dec not found in catalog.")
    return ra, dec


def load_catalog(path):
    with open(path,"r",encoding="utf-8") as f:
        R = csv.DictReader(f)
        ra, dec = detect_columns(R.fieldnames)
        RA, Dec = [], []
        for row in R:
            RA.append(float(row[ra]))
            Dec.append(float(row[dec]))
    return np.array(RA), np.array(Dec)


# ------------------------------------------------------------
# RA/Dec → galactic xyz and (l,b)
# ------------------------------------------------------------

def radec_to_xyz(RA,Dec):
    RA = np.radians(RA)
    Dec= np.radians(Dec)
    x = np.cos(Dec)*np.cos(RA)
    y = np.cos(Dec)*np.sin(RA)
    z = np.sin(Dec)
    return np.vstack([x,y,z]).T


def equatorial_to_galactic_matrix():
    return np.array([
        [-0.054875539390, -0.873437104725, -0.483834991775],
        [ 0.494109453633, -0.444829594298,  0.746982248696],
        [-0.867666135681, -0.198076389622,  0.455983794523],
    ])


def xyz_to_lb(Xgal):
    x,y,z = Xgal[:,0],Xgal[:,1],Xgal[:,2]
    b = np.degrees(np.arcsin(np.clip(z,-1,1)))
    l = np.degrees(np.arctan2(y,x)) % 360.0
    return l,b


def radec_to_galactic(RA,Dec):
    Xeq = radec_to_xyz(RA,Dec)
    M = equatorial_to_galactic_matrix()
    Xgal = Xeq @ M.T
    Xgal /= (np.linalg.norm(Xgal,axis=1,keepdims=True)+1e-15)
    l,b = xyz_to_lb(Xgal)
    return Xgal,l,b


# ------------------------------------------------------------
# random isotropic restricted to a galactic quadrant
# ------------------------------------------------------------

def random_isotropic_quadrant(N, qid, rng):
    """
    generate N isotropic sky positions restricted to a given quadrant.
    quadrants defined by ranges in (l,b).
    """
    lam = np.zeros(N)
    bet = np.zeros(N)

    if qid == 1:   # l in [0,180), b>=0
        lmin,lmax = 0,180
        bsign = +1
    elif qid == 2: # l in [0,180), b<0
        lmin,lmax = 0,180
        bsign = -1
    elif qid == 3: # l in [180,360), b>=0
        lmin,lmax = 180,360
        bsign = +1
    else:          # qid == 4
        lmin,lmax = 180,360
        bsign = -1

    for i in range(N):
        # uniform in l
        lam[i] = rng.uniform(lmin,lmax)

        # uniform in sin(b)
        z = rng.uniform(0,1) if bsign>0 else rng.uniform(-1,0)
        bet[i] = np.degrees(np.arcsin(z))

    # convert to xyz
    l_rad = np.radians(lam)
    b_rad = np.radians(bet)

    Xgal = np.vstack([
        np.cos(b_rad)*np.cos(l_rad),
        np.cos(b_rad)*np.sin(l_rad),
        np.sin(b_rad)
    ]).T

    return Xgal


# ------------------------------------------------------------
# remnant sign and Ylm for phase memory
# ------------------------------------------------------------

def remnant_sign(Xgal, axis_vec):
    axis = axis_vec/np.linalg.norm(axis_vec)
    return np.where(Xgal @ axis > 0, 1, -1)


def legendre_P(l,m,x):
    m0 = abs(m)
    Pmm = np.ones_like(x)
    if m0>0:
        somx2=np.sqrt(1-x*x)
        fact=1.0
        for _ in range(m0):
            Pmm *= -fact*somx2
            fact += 2.0
    if l==m0:
        return Pmm
    Pm1m = x*(2*m0+1)*Pmm
    if l==m0+1:
        return Pm1m
    for ll in range(m0+2, l+1):
        Pll = ((2*ll-1)*x*Pm1m - (ll+m0-1)*Pmm)/(ll-m0)
        Pmm, Pm1m = Pm1m, Pll
    return Pll


def Ylm(l,m,theta,phi):
    x = np.cos(theta)
    Plm = legendre_P(l,m,x)
    K = math.sqrt((2*l+1)/(4*math.pi) *
                  math.factorial(l-abs(m)) /
                  math.factorial(l+abs(m)))
    if m>0:
        return math.sqrt(2)*K*Plm*np.cos(m*phi)
    elif m<0:
        return math.sqrt(2)*K*Plm*np.sin(abs(m)*phi)
    else:
        return K*Plm


def compute_Ylm_matrix(X,lmax=8):
    theta = np.arccos(np.clip(X[:,2],-1,1))
    phi   = np.mod(np.arctan2(X[:,1],X[:,0]), 2*np.pi)
    Y = np.zeros((len(X),(lmax+1)*(lmax+1)))
    idx=0
    for l in range(lmax+1):
        for m in range(-l,l+1):
            Y[:,idx] = Ylm(l,m,theta,phi)
            idx+=1
    return Y


def phase_memory(Y,sgn):
    pos = (sgn>0)
    neg = (sgn<0)
    if np.sum(pos)<2 or np.sum(neg)<2:
        return np.nan
    Apos = np.sum(Y[pos],axis=0)
    Aneg = np.sum(Y[neg],axis=0)
    dphi = np.abs(np.arctan2(Apos-Aneg, np.ones_like(Apos)))
    return np.mean(dphi)


# ------------------------------------------------------------
# main
# ------------------------------------------------------------

def main(path, n_iso=500, seed=222):
    print("[info] loading catalog…")
    RA,Dec = load_catalog(path)

    print("[info] converting to galactic (xyz,l,b)…")
    Xgal,l,b = radec_to_galactic(RA,Dec)

    axis = np.array([
        np.cos(np.radians(-0.5))*np.cos(np.radians(159.8)),
        np.cos(np.radians(-0.5))*np.sin(np.radians(159.8)),
        np.sin(np.radians(-0.5))
    ])

    quadrants = {
        1: ( (l>=0)&(l<180) & (b>=0) ),
        2: ( (l>=0)&(l<180) & (b<0) ),
        3: ( (l>=180)&(l<360) & (b>=0) ),
        4: ( (l>=180)&(l<360) & (b<0) )
    }

    rng = np.random.RandomState(seed)
    results = []

    for qid in [1,2,3,4]:
        print(f"[info] --- quadrant Q{qid} ---")
        mask = quadrants[qid]
        Xc = Xgal[mask]
        N = len(Xc)
        print(f"[info] FRBs in Q{qid}: N={N}")

        if N<15:
            print(f"[warn] Q{qid}: too few FRBs, skipping.")
            continue

        sgn_real = remnant_sign(Xc, axis)
        Y_real = compute_Ylm_matrix(Xc, lmax=8)
        Z_real = phase_memory(Y_real, sgn_real)
        print(f"[info] Z_real={Z_real:.6f}")

        Z_iso_list=[]
        for _ in tqdm(range(n_iso), desc=f"iso Q{qid}", leave=False):
            Xiso = random_isotropic_quadrant(N, qid, rng)
            sgn_iso = remnant_sign(Xiso, axis)
            Y_iso = compute_Ylm_matrix(Xiso, lmax=8)
            Z_iso = phase_memory(Y_iso, sgn_iso)
            Z_iso_list.append(Z_iso)

        Z_iso = np.array(Z_iso_list)
        iso_mean=float(np.mean(Z_iso))
        iso_std =float(np.std(Z_iso))
        p_geom = (1+np.sum(Z_iso>=Z_real))/(len(Z_iso)+1)

        print(f"[info] iso_mean={iso_mean:.6f} iso_std={iso_std:.6f} p_geom={p_geom:.6f}")

        results.append({
            "qid":qid,
            "N":N,
            "Z_real":float(Z_real),
            "iso_mean":iso_mean,
            "iso_std":iso_std,
            "p_geom":float(p_geom)
        })

    print("===============================================================")
    print(" REMNANT-TIME PHASE-MEMORY — QUADRANT TEST (85M) ")
    print("===============================================================")
    for r in results:
        print(f"Q{r['qid']}: N={r['N']:4d}  "
              f"Z_real={r['Z_real']:.6f}  "
              f"iso_mean={r['iso_mean']:.6f}  "
              f"iso_std={r['iso_std']:.6f}  "
              f"p_geom={r['p_geom']:.6f}")
    print("===============================================================")
    print("interpretation:")
    print("  if Z_real > iso_mean in each quadrant with small p_geom, the")
    print("  phase-memory signal is spatially distributed, not confined")
    print("  to any region or scanning footprint. this is a locality-break")
    print("  test — surviving it is extremely strong evidence.")
    print("===============================================================")
    print("test 85M complete.")
    print("===============================================================")


if __name__=="__main__":
    if len(sys.argv)<2:
        print("usage: python frb_remnant_time_phase_memory_quadrants_real_vs_iso_test85M.py frbs_unified.csv")
        sys.exit(1)
    main(sys.argv[1])
